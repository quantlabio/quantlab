 ...
 */
export
const ISheetTools = new Token<ISheetTools>('jupyter.services.sheet-tools');
/* tslint:enable */
 ...
 */
export
interface ISheetTools extends SheetTools {};
 ...
 */
export
class SheetTools extends Widget {
  /**
   * Construct a new CellTools object.
   */
  constructor(options: SheetTools.IOptions) {
    super();
   ...
   */
  get selectedCells(): string[] {
    let selected: string[] = [];
    let panel = this._tracker.currentWidget;
    if (!panel) {
      return selected;
    }
    //each(panel.widgets, widget => {
   ...
   */
  addItem(options: SheetTools.IAddOptions): void {
    ...
    MessageLoop.sendMessage(tool, SheetTools.ActiveCellMessage);
  }
  ...
  private _onActiveCellChanged(): void {
    ...
    each(this.children(), widget => {
      MessageLoop.sendMessage(widget, SheetTools.ActiveCellMessage);
    });
  }
  ...
  private _onSelectionChanged(): void {
    each(this.children(), widget => {
      MessageLoop.sendMessage(widget, SheetTools.SelectionMessage);
    });
  }
...
/**
 * The namespace for SheetTools class statics.
 */
export
namespace SheetTools {
  ...
  interface IOptions {
    /**
     * The spreadsheet tracker used by the sheet tools.
     */
    tracker: ISpreadsheetTracker;
  }
   ...
   */
  export
  const ActiveCellMessage = new ConflatableMessage('sheet-activecell-changed');

  /**
   * A singleton conflatable `'selection-changed'` message.
   */
  export
  const SelectionMessage = new ConflatableMessage('sheet-selection-changed');
...
namespace Private {
   ...
   */
  export
  function createSelectorNode(options: SheetTools.KeySelector.IOptions): HTMLElement {
