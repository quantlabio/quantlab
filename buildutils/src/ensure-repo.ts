...
let MISSING: { [key: string]: string[] } = {
  '@quantlab/buildutils': ['path']
};

let UNUSED: { [key: string]: string[] } = {
  '@quantlab/apputils': ['@types/react'],
  '@quantlab/apputils-extension': ['es6-promise'],
  '@quantlab/theme-dark-extension': ['font-awesome'],
  '@quantlab/theme-light-extension': ['font-awesome'],
};
...
/**
 * Ensure the quantlab application package.
 */
function ensureQuantlab(): string[] {
  ...
  corePackage.quantlab.extensions = {};
  corePackage.quantlab.mimeExtensions = {};
  corePackage.quantlab.version = version;
  corePackage.quantlab.linkedPackages = {};
  corePackage.dependencies = {};

  let singletonPackages = corePackage.quantlab.singletonPackages;
  ...
  utils.getCorePaths().forEach(pkgPath => {
    ...
    corePackage.quantlab.linkedPackages[data.name] = relativePath;
    ...
    ['extension', 'mimeExtension'].forEach(item => {
      ...
      corePackage.quantlab[item + 's'][data.name] = ext;
    });
  });
...
function ensureIntegrity(): boolean {
  ...
  // in sync.
  //paths.push('./quantlab/tests/mock_packages/extension');
  //paths.push('./quantlab/tests/mock_packages/mimeextension');
  ...
  if (pkgMessages.length > 0) {
    let pkgName ='@quantlab/metapackage';
    if (!messages[pkgName]) {
      messages[pkgName] = [];
    }
  ...
  // Handle the QuantLab application top package.
  pkgMessages = ensureQuantlab();
  if (pkgMessages.length > 0) {
    let pkgName = '@quantlab/application-top';
    if (!messages[pkgName]) {
      messages[pkgName] = [];
    }
    messages[pkgName] = messages[pkgName].concat(pkgMessages);
  ...
  if (Object.keys(messages).length > 0) {
    ...
    if (process.env.TRAVIS_BRANCH || process.env.APPVEYOR) {
      console.log('\n\nPlease run `qlpm run integrity` locally and commit the changes');
      process.exit(1);
    }
    utils.run('qlpm install');
