...
def find_package_data():
    ...
    """
    theme_dirs = []
    for dir, subdirs, files in os.walk(pjoin('quantlab', 'themes')):
        slice_len = len('quantlab' + os.sep)
        theme_dirs.append(pjoin(dir[slice_len:], '*'))

    return {
        'quantlab': ['build/*', 'schemas/*', 'index.app.js',
...
class CoreDeps(Command):
    ...
    targets = [
        pjoin(here, 'quantlab', 'schemas', 'jupyter.extensions.shortcuts.json'),
        pjoin(here, 'quantlab', 'themes', 'quantlab-theme-light-extension',
            'images', 'quantlab.svg')
    ]
    ...
    def run(self):
        # Handle schemas.
        schema = pjoin(here, 'quantlab', 'schemas')
        ...
        for pkg in packages:
            ...
            if 'quantlab' not in data:
                continue
            schemaDir = data['quantlab'].get('schemaDir', '')
            if schemaDir:
        ...
        # Handle themes
        themes = pjoin(here, 'quantlab', 'themes')
        ...
        for pkg in packages:
            ...
            if 'quantlab' not in data:
                continue
            themeDir = data['quantlab'].get('themeDir', '')
            if themeDir:
